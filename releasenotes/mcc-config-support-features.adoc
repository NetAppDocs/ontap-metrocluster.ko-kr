...
ONTAP 9.14.1
 ...
 a| 
ONTAP mediator 1.5는 MetroCluster IP 구성에서 지원됩니다
 a| 
...
ONTAP 9.11.1
 ...
 a| 
ONTAP mediator는 MetroCluster IP 구성에서 지원됩니다
 a| 
ONTAP mediator 서비스는 상태 정보에 대한 물리적으로 분리된 리포지토리를 제공하여 MetroCluster IP 구성에서 예기치 않은 자동 전환을 수행할 수 있도록 지원할 수 있습니다.

https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_configuring_the_ontap_mediator_service_from_a_metrocluster_ip_configuration.html["계획되지 않은 자동 전환을 위한 ONTAP 중재자 서비스 구성"]
 a| 
ONTAP 9.7 을 참조하십시오
