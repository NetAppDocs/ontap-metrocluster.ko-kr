...
Tiebreaker 소프트웨어의 관찰자 모드를 비활성화하면 사이트 장애가 감지된 후 자동으로 전환이 시작됩니다.


NOTE: Tiebreaker 1.6 이상 버전에서는 모니터를 추가할 때 자동 전환 기능을 활성화할 수 있습니다. `monitor add wizard` 명령. 참조하다link:concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["MetroCluster 구성 추가"] .
