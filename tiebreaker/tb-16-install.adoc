...
---
= Tiebreaker 1.6 또는 1.6P1을 설치하세요.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
호스트 Linux 운영 체제에서 Tiebreaker 1.6 또는 tiebreaker 1.6P1로 새로 설치하거나 업그레이드하여 MetroCluster 구성을 모니터링합니다.
...
NOTE: Tiebreaker 1.6 이상 버전을 설치한 후에는 link:concept_configuring_the_tiebreaker_software.html#import-certificates-tb17["인증서 가져오기"] 끊김 없는 모니터링을 활성화해야 합니다.



== Tiebreaker 1.6P1을(를) 설치하거나 업그레이드합니다

Tiebreaker 1.6P1을 설치하거나 Tiebreaker 1.6, 1.5 또는 1.4에서 Tiebreaker 1.6P1로 업그레이드할 수 있습니다.

.단계
. MetroCluster Tiebreaker 1.6P1 소프트웨어를 다운로드합니다.
+
...
. 호스트에 루트 사용자로 로그인합니다.
. 업그레이드를 수행하는 경우 실행 중인 Tiebreaker 버전을 확인합니다.
+
다음 예에서는 Tiebreaker 1.5를 보여 줍니다.
...
[role="tabbed-block"]
====
.Tiebreaker 1.6P1을 설치합니다
--
Tiebreaker 1.6P1을 새로 설치하려면 다음 단계를 따르십시오.
...
--
.1.6을 1.6P1로 업그레이드합니다
--
다음 단계에 따라 Tiebreaker 1.6 소프트웨어 버전을 Tiebreaker 1.6P1로 업그레이드하십시오.


NOTE: 1.6에서 Tiebreaker 1.6P1로 업그레이드한 후 기존 모니터를 제거하고 모니터링을 위해 MetroCluster 구성을 다시 추가합니다.
...
----
=====
.. 의 단계에 따라 MetroCluster 구성을 제거했다가 다시 추가합니다link:concept_configuring_the_tiebreaker_software.html["Tiebreaker 소프트웨어를 구성합니다"].


--
.1.5를 1.6P1로 업그레이드합니다
--
다음 단계에 따라 Tiebreaker 1.5 소프트웨어 버전을 Tiebreaker 1.6P1로 업그레이드하십시오.
...
--
.1.4를 1.6P1로 업그레이드합니다
--
다음 단계에 따라 Tiebreaker 1.4 소프트웨어 버전을 Tiebreaker 1.6P1로 업그레이드하십시오.
...
== Tiebreaker 1.6를 설치하거나 업그레이드합니다

Tiebreaker 1.6을 설치하거나 Tiebreaker 1.5 또는 1.4에서 Tiebreaker 1.6으로 업그레이드할 수 있습니다.

.단계
. MetroCluster Tiebreaker 1.6 소프트웨어를 다운로드합니다.
+
https://mysupport.netapp.com/site/products/all/details/metrocluster-tiebreaker/downloads-tab["MetroCluster Tiebreaker (다운로드) - NetApp Support 사이트"^]
...
.타이브레이커 1.6을 설치합니다
--
Tiebreaker 1.6을 새로 설치하려면 다음 단계를 따르십시오.
...
.1.5를 1.6으로 업그레이드합니다
--
다음 단계에 따라 Tiebreaker 1.5 소프트웨어 버전을 Tiebreaker 1.6으로 업그레이드합니다.
...
.1.4를 1.6으로 업그레이드합니다
--
다음 단계에 따라 tiebreaker 1.4 소프트웨어 버전을 tiebreaker 1.6으로 업그레이드합니다.
...
.다음 단계
Tiebreaker를 성공적으로 설치 또는 업그레이드한 후 link:configure-tb-workflow.html["MetroCluster Tiebreaker 소프트웨어를 구성합니다"] MetroCluster 구성을 추가하고 필요한 인증서를 가져오십시오.
