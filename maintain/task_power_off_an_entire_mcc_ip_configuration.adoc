...
[role="lead"]
유지보수 또는 이전 작업을 시작하기 전에 전체 MetroCluster IP 구성과 모든 장비의 전원을 끄십시오.
...
. 양쪽 사이트에서 MetroCluster 구성을 확인하십시오.
+
...
NOTE: MetroCluster IP 구성에서 ONTAP Mediator를 사용하는 구성이 아닌 경우 AUSO 장애 도메인은 이미 'auso-disabled'로 설정되어 있습니다.
