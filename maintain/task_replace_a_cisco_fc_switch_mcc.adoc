...
+
.. 교체 스위치를 켜고 부팅하십시오.
.. 교체 스위치의 펌웨어 버전을 확인하여 다른 FC 스위치의 버전과 일치하는지 확인합니다
...
----
.. 교체 스위치에서 정상적인 스위치에서 조닝 정보가 올바르게 검색되었는지 확인합니다. + " * show zone * "
+
[listing]
----
...
+
이 예에서 출력에 "멤버 인터페이스 없음"은 다음 멤버가 Fabric에 있는 스위치 중 하나의 스위치 WWN과 연결되어 있지 않으므로 제거해야 함을 보여 줍니다.
...
----
 ...
 FC_switch_B_1# copy running-config startup-config
----
.. 조닝이 올바르게 구성되었는지 확인합니다. + " * show zone * "
+
