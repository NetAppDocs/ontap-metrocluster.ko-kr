...
+
image::../media/mcc_ip_hardware_architecture_ip_interconnect.png[스위치 간 링크 및 스위치 유형을 보여주는 MetroCluster IP 하드웨어 아키텍처]
...
image:../media/mcc_port_usage_workflow.png["스위치 교체 중 MetroCluster IP 포트 사용 및 케이블 변경에 대한 워크플로"]
...
----
. 현재 홈 포트에 없는 모든 클러스터 LIF를 홈 포트로 되돌립니다.
+
