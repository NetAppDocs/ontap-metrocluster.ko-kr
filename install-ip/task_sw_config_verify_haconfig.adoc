...
+
컨트롤러 모듈 및 섀시에는 "mccip" 값이 표시되어야 합니다.

. 컨트롤러의 표시된 시스템 상태가 "mcip"이 아닌 경우 컨트롤러에 대한 HA 상태를 설정합니다.
+
ha-config modify controller mcip.(컨트롤러 mccip 수정

. 표시된 섀시 시스템 상태가 "mcip"이 아닌 경우 섀시의 HA 상태를 설정합니다.
+
...
. 부팅 메뉴의 옵션 "4"를 사용하여 사이트 A에서 각 노드를 부팅합니다.
+
...
. 부팅 메뉴의 옵션 "4"를 사용하여 사이트 B에서 각 노드를 부팅합니다.
+
다음 노드로 진행하기 전에 노드에서 이 단계를 완료해야 합니다.
...
새 컨트롤러를 netboot에 연결해야 하는 경우 를 참조하십시오 link:../upgrade/task_upgrade_controllers_in_a_four_node_ip_mcc_us_switchover_and_switchback_mcc_ip.html#netbooting-the-new-controllers["새 컨트롤러 모듈을 Netbooting 합니다"] MetroCluster 업그레이드, 전환 및 확장 가이드 _.
...
2 entries were displayed.
----
. DR 그룹 'MetroCluster configuration-settings dr-group create-partner-cluster_partner-cluster-name_-local-node_local-node-name_-remote-node_remote-node-name_'을 생성합니다
+
...
--
+
.. node_A_1의 포트 "e5a"에서 인터페이스를 구성합니다.
+
...
----
+
MetroCluster IP 인터페이스에 대한 VLAN을 지원하는 플랫폼 모델에서 기본 VLAN ID를 사용하지 않으려면 '-vlan-id' 매개 변수를 포함할 수 있습니다. 다음 예에서는 AFF ID가 "120"인 VLAN A220 시스템에 대한 명령을 보여 줍니다.
...
--
+
.. "node_B_2"의 포트 "e5a"에서 인터페이스를 구성합니다.
+
'MetroCluster configuration-settings interface create-cluster-name_cluster-name_-home-node_node-name_-home-port e5a-address_ip-address_-netmask_mask_'
...
+
드라이브 자동 할당에서는 "node_A_1"에 쿼터 1개(드라이브 8개)를, "node_A_2"에 분기당 1개를 할당했습니다. 나머지 드라이브는 "node_B_1" 및 "node_B_2"에 대한 원격(pool1) 디스크입니다.
...
--
+
다음 출력에서는 쉘프 "23"의 드라이브가 더 이상 할당되지 않은 드라이브 목록에 표시되지 않으므로 할당된 드라이브를 보여 줍니다.
...
cluster_A::>
----
.. 이 단계를 반복하여 사이트 A의 두 번째 노드에 풀 1 드라이브를 할당합니다(예: "n" ode_A_2").
.. 사이트 B에서 이 단계를 반복합니다
...
--
+
다음 출력에서는 쉘프 "23"의 디스크가 이제 할당되었으며 더 이상 표시되지 않음을 보여 줍니다.
...
* 컨트롤러 및 섀시의 ha-config 상태는 mcip이어야 합니다.
...
----
. 사이트 A:'network port show'의 네트워킹 상태를 확인합니다
+
