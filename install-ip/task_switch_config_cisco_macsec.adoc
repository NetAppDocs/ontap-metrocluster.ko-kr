...
summary: 필요한 경우 사이트 간에 실행되는 WAN ISL 포트에만 MACsec 암호화를 구성하십시오. MACsec 암호화를 사용하도록 선택한 경우 올바른 RCF 파일을 적용한 후 구성하십시오. 
---
= MetroCluster IP 사이트의 Cisco 9336C 스위치에서 MACsec 암호화를 구성합니다(선택 사항)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
필요한 경우 사이트 간에 실행되는 WAN ISL 포트에서 MACsec 암호화를 구성할 수 있습니다. MACsec 암호화를 사용하도록 선택한 경우 올바른 RCF 파일을 적용한 후 구성해야 합니다.

[NOTE]
====
* MACsec 암호화는 사이트 간에 실행되는 WAN ISL 포트에만 적용할 수 있습니다.
* Cisco 9336C 스위치를 구성할 때 MACsec 암호화 구성은 선택 사항입니다.
