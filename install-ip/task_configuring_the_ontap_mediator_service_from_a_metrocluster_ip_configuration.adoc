...
* 이 작업은 MetroCluster IP 구성의 모든 노드에 있는 ONTAP 인터페이스에서 수행할 수 있습니다.
* ONTAP 9.18.1 및 ONTAP Mediator 1.11 버전부터 단일 ONTAP Mediator 인스턴스에서 최대 10개의 MetroCluster 구성을 동시에 관리할 수 있습니다. 이전 버전에서는 ONTAP Mediator가 최대 5개의 MetroCluster 구성을 동시에 지원할 수 있었습니다.
...
NOTE: Mediator 관리자 사용자 계정의 사용자 이름과 암호를 입력하라는 메시지가 표시됩니다.
...
. 선택적으로 MetroCluster 자동 강제 전환을 구성할 수 있습니다.
+
다음 명령은 고급 권한 수준에서만 사용할 수 있습니다.

+

IMPORTANT: 이 명령을 사용하기 전에 link:concept-ontap-mediator-supports-automatic-unplanned-switchover.html#mauso-9-12-1["MetroCluster 자동 강제 적용 시 위험 및 제한 사항"]을(를) 검토하십시오.
