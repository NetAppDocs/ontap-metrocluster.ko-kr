...
+
** AFF A1K 및 AFF A90 시스템에서는 ISL 속도가 최소 100Gbps여야 합니다. 다른 ISL 속도를 사용하려면 FPVR(Feature Product Variance Request)이 필요합니다. FPVR을 제출하려면 영업팀에 문의하세요.
** 다른 모든 플랫폼에서는 ISL 속도가 최소 10Gbps여야 합니다.
