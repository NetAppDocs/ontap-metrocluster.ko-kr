...
|===
+
** 방법 1:  to upgrade EFOS with downloading the software image to the backup boot partition
** 방법 2:  to upgrade EFOS using the ONIE OS installation
...
NOTE: 드문 경우지만 스위치가 부팅되지 않을 수 있습니다. 를 따릅니다  to upgrade EFOS using the ONIE OS installation 새 이미지를 설치합니다.
...
+
.. 스위치 기본 설정을 구성합니다. 4단계 이후 단계를 따릅니다.  the Broadcom IP switch to factory defaults
.. 에 설명된 대로 RCF 파일을 생성하고 적용합니다  and installing the Broadcom RCF files
