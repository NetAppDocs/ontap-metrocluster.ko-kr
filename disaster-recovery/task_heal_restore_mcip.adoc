...
ONTAP 9.6부터 재해 사이트 노드가 부팅될 때 복구 작업이 자동으로 수행됩니다. 복구 명령은 필요하지 않습니다.
...
xref:task_complete_recovery.html#verifying-licenses-on-the-replaced-nodes[교체된 노드에서 라이센스 확인]
