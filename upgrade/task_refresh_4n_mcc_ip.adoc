...
+
image::../media/mcc_dr_group_a.png[확장 전 MetroCluster IP 4노드 구성]

+
8노드 구성은 다음 이미지와 같이 나타납니다.

+
image::../media/mcc_dr_groups_8_node.gif[확장 후 8개 노드로 구성된 MetroCluster IP 구성]
...
+
다음 예에서는 10시간의 유지 관리 기간을 지정합니다.  귀하의 계획에 따라 추가 시간을 허용하세요.
...
+
.일시적인 8노드 구성
image::../media/mcc_dr_group_b.png[확장 및 CRS 볼륨 마이그레이션 후 MetroCluster 구성]

+
.임시 12노드 구성
image::../media/mcc_dr_group_c4.png[임시 12노드 MetroCluster 구성]
...
. 8노드 구성에서 두 DR 그룹을 모두 새로 고쳐야 하는 경우 각 DR 그룹에 대해 전체 절차를 반복합니다.
+
이전 DR 그룹을 제거한 후 다음 이미지와 같이 구성이 나타납니다.

+
.4노드 구성
image::../media/mcc_dr_group_d.png[이전 DR 그룹을 제거한 후 MetroCluster 구성]

+
.8노드 구성
image::../media/mcc_dr_group_c5.png[최종 8노드 MetroCluster 구성]
...
Tiebreaker입니다
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-metrocluster-configurations["MetroCluster 구성 추가"] _MetroCluster Tiebreaker 설치 및 구성_에서.
