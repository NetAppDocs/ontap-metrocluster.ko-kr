...
* 지원되는 플랫폼 업그레이드 조합에 대한 자세한 내용은 의 MetroCluster FC 업그레이드 표를 참조하십시오 link:concept_choosing_controller_upgrade_mcc.html#metrocluster-fc-controller-upgrades["컨트롤러 업그레이드 절차를 선택합니다"].


을 참조하십시오 https://["업그레이드 또는 새로 고침 방법 선택"] 을 참조하십시오.
...
* 이 절차를 사용하면 통계청 기반의 자동 스위치오버 및 스위치백을 사용하여 4노드 MetroCluster FC 구성에서 컨트롤러를 업그레이드할 수 있습니다. ARL(Aggregate Relocation)을 사용하여 컨트롤러 업그레이드를 수행하려면 을 참조하십시오 https://[""시스템 컨트롤러 교체" 명령을 사용하여 ONTAP 9.8 이상을 실행하는 컨트롤러 하드웨어를 업그레이드합니다"]. 통계청 기반의 자동화 절차를 사용하는 것이 좋습니다.
...
+
자세한 내용은 을 참조하십시오 https://["온보드 키 관리 정보를 수동으로 백업합니다"^].
...
.단계
. http://["기존 MetroCluster 구성을 제거합니다"^] Tiebreaker 소프트웨어
...
* 에서 수집한 정보를 사용합니다 <<업그레이드를 준비하는 중입니다>>.
...
+
플랫폼 모델의 포트 사용은 를 참조하십시오 https://["NetApp Hardware Universe를 참조하십시오"^]
...
+
https://["브로드캐스트 도메인에서 포트 추가 또는 제거"^]
...
+
https://["VLAN을 생성하는 중입니다"^]

+
https://["물리적 포트를 결합하여 인터페이스 그룹을 생성합니다"^]
...
자세한 내용은 을 참조하십시오 https://["온보드 키 관리 암호화 키를 복원하는 중입니다"^].
...
자세한 내용은 을 참조하십시오 https://["외부 키 관리 암호화 키 복원"^].
...
. 의 단계를 사용합니다 http://["MetroCluster 구성 추가"].
