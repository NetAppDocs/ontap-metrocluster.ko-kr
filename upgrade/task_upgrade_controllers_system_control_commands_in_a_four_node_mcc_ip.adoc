...
이 절차는 FAS8200에서 FAS9500까지, 그리고 AFF A900 컨트롤러 업그레이드에 대한 AFF A300을 지원합니다.
...
사전 점검을 시작하기 전에 ONTAP 중재자가 설치된 경우 자동으로 감지되어 제거됩니다. 제거를 확인하려면 사용자 이름과 암호를 입력하라는 메시지가 표시됩니다. 업그레이드를 완료하거나 사전 점검을 통과하지 못하거나 업그레이드를 진행하지 않도록 선택하는 경우 반드시 다음을 수행해야 합니다 link:task_upgrade_controllers_system_control_commands_in_a_four_node_mcc_ip.html#reconfigure-ontap-mediator["ONTAP 중재자를 수동으로 재구성합니다"].
