...
. 업그레이드 대상 시스템이 다음 표에 없는 경우 MetroCluster IP 인터페이스에서 지원되는 IP 주소를 사용하고 있는지 확인하십시오.
+
|===
...
CAUTION: MetroCluster 인터페이스가 169.254.17.x 또는 169.254.18.x IP 주소로 구성된 경우 표에 나열되지 않은 시스템으로의 업그레이드는 지원되지 않습니다.
