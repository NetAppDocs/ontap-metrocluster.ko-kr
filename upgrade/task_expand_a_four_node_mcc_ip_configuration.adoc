...
. 유지보수 모드에서 halt 명령을 실행하여 유지보수 모드를 종료합니다.
+
"중지"
...
+
전체 명령 구문은 link:https://docs.netapp.com/us-en/ontap-cli["ONTAP 명령 참조"^]을 참조하십시오.
...
+
다음 예는 SVM "cluster01"의 페일오버 그룹 "intercluster01"에 포트 "e0e" 및 "e0f"를 할당합니다.
...
+
다음 예는 포트 "e0e"의 인터클러스터 LIF "cluster01_icl01" 및 "cluster01_icl02"가 포트 "e0f"로 페일오버됨을 보여줍니다.
...
== MetroCluster 인터페이스와 미러 루트 애그리게이트를 생성합니다

새로운 MetroCluster IP 노드에 MetroCluster 인터페이스를 생성하고 루트 애그리게이트를 미러링해야 합니다.
...
.단계
. 각 사이트에서 새 클러스터 간 LIF를 피어 관계에 추가합니다.
+
...
NOTE: The `metrocluster configuration-settings connection show` 명령어의 출력 결과는 플랫폼 모델에 따라 다를 수 있습니다. 예를 들어, 일부 플랫폼에서는 HA 파트너 관련 항목이 출력에 표시되지 않을 수 있습니다.
...
[NOTE]
====
** 사이트당 하나 이상의 미러링된 데이터 애그리게이트를 만들어야 합니다. MetroCluster IP 노드에서 사이트당 두 개의 미러링된 데이터 집계를 사용하여 MDV 볼륨을 호스팅하는 것이 좋지만 사이트당 단일 집계를 지원하는 것은 권장되지 않습니다. MetroCluster의 한 사이트에 단일 미러링된 데이터 애그리게이트가 있고 다른 사이트에 둘 이상의 미러링된 데이터 애그리게이트가 있는 것은 허용됩니다.
