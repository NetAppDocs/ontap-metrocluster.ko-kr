...
ONTAP 9.13.1 부터는 MetroCluster IP 구성에 세 번째 DR 그룹으로 네 개의 새 노드를 추가할 수 있습니다. 그러면 12노드 MetroCluster 구성이 생성됩니다.
...
* 이 절차에서는 4노드 DR 그룹 하나를 확장하는 데 필요한 단계를 설명합니다. 8노드 구성(DR 그룹 2개)이 있는 경우 각 DR 그룹에 대해 전체 절차를 반복해야 합니다.
...
* 이 절차는 FAS2750 또는 AFF A220 시스템에서 지원되지 않습니다.
...
====
image::../media/8node_comb_ip.png[8노드 구성을 위한 플랫폼 조합]
