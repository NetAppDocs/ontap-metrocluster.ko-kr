...
.단계
. 새 MetroCluster IP 노드에서 _MetroCluster IP 설치 및 구성 가이드 _ 의 절차를 사용하여 인터클러스터 LIF를 구성합니다.
+
...
+
MetroCluster 구성 설정 및 연결에 대한 자세한 내용은 _MetroCluster IP 설치 및 구성 안내서_를 참조하십시오.
...
NOTE: 두 클러스터 중 하나에서 MetroCluster IP 인터페이스를 구성할 수 있습니다. 또한 ONTAP 9.1.1부터 Layer 3 구성을 사용하는 경우 MetroCluster IP 인터페이스를 만들려면 '-gateway' 매개 변수도 지정해야 합니다. 을 참조하십시오 link:..install-ip/concept_considerations_layer_3.html["계층 3 광역 네트워크에 대한 고려 사항"].
