...
* 참고 3: 통합 시스템(동일한 섀시의 디스크 및 컨트롤러)을 업그레이드하려면 기존 섀시와 디스크는 그대로 두고 컨트롤러 모듈만 교체해야 합니다.
* 참고 4: 기존 컨트롤러를 외부 SAS 셸프로 변환하려면 IOM 모듈이 필요합니다. 지원되는 IOM 모듈은 link:https://hwu.netapp.com/["Hardware Universe"^]을 참조하십시오.
* 참고 5: ONTAP 9.18.1GA 이상이 필요합니다.
...
image:../media/assisted-group-2-a70-a90-updated.png["AFF 및 FAS 업그레이드 조합 그룹 2를 보여줍니다."]
