...
NOTE: 전환 및 시스템 새로 고침 워크플로 중에 이러한 단계를 사용합니다. 이러한 워크플로의 일부로 이 절차의 일부 단계를 이미 완료했을 수 있습니다.
...
NOTE: MetroCluster remove-dr-group 명령은 ONTAP 9.8 이상에서만 지원됩니다.
...
+
MetroCluster FC 구성에서 <<fc_step_7_remove_nodes,기존 DR 그룹 노드 제거>> 단계로 이동합니다.
...
.. 삭제할 각 DR 그룹 노드에서 pool1 디스크의 소유권을 제거합니다. 삭제할 각 노드에 대해 다음 단계를 수행하십시오. 이 단계의 예에서는 node_A_1을 삭제합니다.
+
... 노드셸로 이동합니다.
+
`run -node <node_name>`
...
+
노드가 소유한 Pool1 디스크를 포함하여 모든 디스크가 표시됩니다. 다음 예는 디스크 0n.4와 해당 파티션이 Pool1에 할당된 경우의 축약된 출력입니다. 출력은 구성에 따라 달라집니다.
...
... 각 pool1 디스크에 대한 디스크 소유권을 제거합니다.
+
구성에 파티션된 디스크가 포함된 경우 먼저 각 파티션된 디스크의 소유권을 제거한 다음 컨테이너 디스크 소유권을 제거합니다.

+
`disk remove_ownership <disk_name>`

+
다음 예에서는 컨테이너 디스크 `0n.4`의 소유권이 제거되기 전에 파티션된 디스크의 소유권이 제거됩니다.
...
. [[fc_step_7_remove_nodes]]이전 DR 그룹의 노드를 제거합니다.
+
...
.관련 정보
이 절차에서 사용된 명령에 대한 자세한 내용은 link:https://docs.netapp.com/us-en/ontap-cli/["ONTAP 명령 참조"^]에서 확인할 수 있습니다.
