...
* 참고 4: 통합 시스템(동일한 섀시의 디스크 및 컨트롤러)을 업그레이드하려면 기존 섀시와 디스크는 그대로 두고 컨트롤러 모듈만 교체해야 합니다.
...
image:../media/manual-upgrade-combination-group-2-a90-fas90-updated.png["에서는 그룹 2 플랫폼에 대해 지원되는 업그레이드 조합을 보여 줍니다"]
...
image::../media/metrocluster_fc_upgrade_table_aff_fas.png[MetroCluster FC 컨트롤러 업그레이드를 위한 지원 플랫폼 조합]
