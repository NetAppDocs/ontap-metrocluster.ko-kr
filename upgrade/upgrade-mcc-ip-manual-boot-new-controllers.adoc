...
|===
+
키 관리자 유형에 따라 첫 번째 부팅 메뉴 프롬프트에서 "`10`" 옵션 또는 "`11`" 옵션을 선택한 다음  `6` 옵션을 선택하여 복구 절차를 수행하십시오. 노드를 완전히 부팅하려면 "`1`" 옵션(정상 부팅)을 선택하여 복구 절차를 반복해야 할 수도 있습니다.

. 교체된 노드가 부팅될 때까지 기다립니다.
+
두 노드 중 하나가 Takeover 모드에 있으면 'storage failover 반환' 명령을 사용하여 Giveback을 수행합니다.

. admin SVM에서 암호화가 활성화된 경우 키 관리 구성에 적합한 명령을 사용하여 키를 복원합니다.
+
|===
