...
NOTE: 클러스터의 모든 노드에 있는 슬롯 7은 에 설명된 대로 비어 있어야 합니다  ports from the old nodes to the new nodes 섹션을 참조하십시오.
...
+
"<path_to_the_web-Accessible_directory>"는 에서 "<ONTAP_version>\_image.tgz"를 다운로드한 위치로 이어져야 합니다 ,2단계.
