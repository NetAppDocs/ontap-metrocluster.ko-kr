...
. MetroCluster 노드가 볼륨 또는 애그리게이트에 암호화를 사용하는 경우 키 및 암호 문구를 복사합니다. 자세한 내용은 을 참조하십시오 https://["온보드 키 관리 정보를 수동으로 백업합니다"^].
+
...
NOTE: 클러스터의 모든 노드에 있는 슬롯 7은 에 설명된 대로 비어 있어야 합니다 <<이전 노드의 포트를 새 노드로 매핑합니다>> 섹션을 참조하십시오.
