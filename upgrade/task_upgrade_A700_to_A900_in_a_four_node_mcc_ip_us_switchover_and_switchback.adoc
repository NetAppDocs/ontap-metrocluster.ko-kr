...
---
= 스위치오버 및 스위치백을 사용하여 MetroCluster IP 구성에서 AFF A700에서 AFF A900으로 컨트롤러 업그레이드(ONTAP 9.10.1 이상)
...
.이 작업에 대해
* 컨트롤러는 ONTAP 9.10.1 이상을 실행해야 합니다.
* 이 절차는 MetroCluster IP 구성에서 AFF A900으로 업그레이드하는 AFF A700 컨트롤러 모듈에 적용됩니다.
+
|===
...
+
AFF A700 및 AFF A900 컨트롤러를 사용하여 MetroCluster 구성을 작동하는 것은 이 유지 관리 활동 외부에서 지원되지 않습니다.
...
AFF A900의 MetroCluster 구성은 슬롯 5와 7에 있는 DR 카드의 각 포트 중 하나를 사용합니다. 업그레이드를 시작하기 전에 AFF A700의 슬롯 7에 카드가 있는 경우 클러스터의 모든 노드에 대해 다른 슬롯으로 카드를 이동해야 합니다.



=== 컨트롤러를 업그레이드하기 전에 MetroCluster 스위치 RCF 파일을 업데이트합니다

AFF A700의 MetroCluster IP 구성에서는 VLAN을 사용하지 않습니다. AFF A900의 MetroCluster 구성은 VLAN을 사용합니다. 따라서 AFF A700에서 AFF A900으로 업그레이드할 때 RCF 파일을 변경해야 합니다.
...
+
** link:../install-ip/task_install_and_cable_the_mcc_components.html#resetting-the-broadcom-ip-switch-to-factory-defaults["Broadcom IP 스위치를 출하 시 기본값으로 재설정합니다"]
** link:../install-ip/task_install_and_cable_the_mcc_components.html#resetting-the-cisco-ip-switch-to-factory-defaults["Cisco IP 스위치를 출하 시 기본값으로 재설정합니다"]
...
+
** link:../install-ip/task_install_and_cable_the_mcc_components.html#downloading-and-installing-the-broadcom-rcf-files["Broadcom RCF 파일 다운로드 및 설치"]
** link:../install-ip/task_install_and_cable_the_mcc_components.html#downloading-and-installing-the-cisco-ip-rcf-files["Cisco IP RCF 파일 다운로드 및 설치"]
...
AFF A700에서 AFF A900으로 업그레이드할 때 데이터 네트워크 포트, FCP SAN 어댑터 포트, SAS 및 NVMe 스토리지 포트는 변경하지 않습니다. 데이터 LIF는 업그레이드 도중과 이후에 계속 유지됩니다. 따라서 이전 노드의 네트워크 포트를 새 노드로 매핑할 필요가 없습니다.
...
Tiebreaker입니다
 a| 
link:install-stretch/concept_considerations_when_removing_mcc_configurations.html["MetroCluster 구성 제거"] MetroCluster Tiebreaker 설치 및 구성 내용 _
...
== AFF A700 플랫폼 컨트롤러 모듈 및 NVS를 제거합니다
...
=== AFF A700 NVS 모듈을 제거합니다

다음 절차를 사용하여 AFF A700 NVS 모듈을 제거합니다.

참고: AFF A700 NVS 모듈 NVS 모듈은 슬롯 6에 있으며 시스템의 다른 모듈에 비해 높이가 2배입니다.
...
. AFF A700 NVS에서 코어 덤프 장치로 사용되는 추가 모듈을 사용하는 경우 AFF A900 NVS로 전송하지 마십시오. AFF A700 컨트롤러 모듈 및 NVS의 부품을 AFF A900으로 전송하지 마십시오.




== AFF A900 NVS 및 컨트롤러 모듈을 설치합니다

업그레이드 키트에서 받은 AFF A900 NVS 및 컨트롤러 모듈을 site_B의 두 노드에 모두 설치해야 합니다 코어 덤프 장치를 AFF A700 NVS 모듈에서 AFF A900 NVS 모듈로 이동하지 마십시오.
...
다음 절차에 따라 site_B의 두 노드의 슬롯 6에 AFF A900 NVS를 설치하십시오
...
=== AFF A900 컨트롤러 모듈을 설치합니다.

다음 절차에 따라 AFF A900 컨트롤러 모듈을 설치하십시오.
...
AFF A900 컨트롤러 모듈과 NVS를 바꾼 후 AFF A900 노드를 netboot 하고 클러스터에서 실행 중인 것과 동일한 ONTAP 버전 및 패치 레벨을 설치해야 합니다. netboot라는 용어는 원격 서버에 저장된 ONTAP 이미지에서 부팅됨을 의미합니다. netboot를 준비할 때 시스템이 액세스할 수 있는 웹 서버에 ONTAP 9 부트 이미지 사본을 추가해야 합니다. AFF A900 컨트롤러 모듈의 부팅 미디어에 설치된 ONTAP 버전은 섀시에 설치하고 전원을 켜지 않는 한 확인할 수 없습니다. AFF A900 부팅 미디어의 ONTAP 버전은 업그레이드할 AFF A700 시스템에서 실행되는 ONTAP 버전과 동일해야 하며 기본 부팅 이미지와 백업 부팅 이미지가 모두 일치해야 합니다. 부팅 메뉴에서 netboot 다음에 'wipecononfig' 명령을 수행하여 이미지를 구성할 수 있습니다. 이전에 다른 클러스터에서 컨트롤러 모듈을 사용한 경우 "wipeconfig" 명령을 실행하면 부팅 미디어의 나머지 구성이 지워집니다.
...
. AFF A900 컨트롤러 모듈에서 지금 실행 중인 node_B_1이 부팅될 때까지 기다린 후 다음과 같이 부팅 메뉴 옵션을 표시합니다.
+
[listing]
----
...
. 부팅 메뉴에서 유지보수 모드로 전환하려면 옵션 '5'를 선택합니다. 유지보수 모드에서 노드가 중지되고 명령 프롬프트 *>가 표시될 때까지 프롬프트에 "yes"를 선택합니다.
...
중재자
 a| 
link:../install-ip/concept_configure_the_ontap_mediator_for_unplanned_automatic_switchover.html["MetroCluster IP 구성에서 ONTAP 중재자 서비스 구성"] MetroCluster IP 설치 및 구성 섹션에서
