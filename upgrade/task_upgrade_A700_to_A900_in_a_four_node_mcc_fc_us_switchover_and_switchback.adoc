...
---
= 스위치오버 및 스위치백을 사용하여 MetroCluster FC 구성에서 AFF A700에서 AFF A900으로 컨트롤러 업그레이드(ONTAP 9.10.1 이상)
:icons: font
:imagesdir: ../media/
...
* 이 절차는 ONTAP 9.10.1 이상에서 사용할 수 있습니다.
+
...
* 업그레이드 절차 중에는 AFF A700 시스템에서 다른 카드를 추가하거나 제거하면 안 됩니다. 자세한 내용은 를 참조하십시오 https://hwu.netapp.com/["NetApp Hardware Universe를 참조하십시오"^]
...
기존 MetroCluster 구성을 변경하기 전에 구성 상태를 확인하고, RCF 파일 및 케이블을 AFF A900 패브릭 MetroCluster 구성에 필요한 새로운 포트 연결 토폴로지에 맞게 변경하고, 기타 작업을 수행해야 합니다.



== AFF A700 컨트롤러에서 슬롯 7을 지웁니다

AFF A900의 MetroCluster 구성에는 슬롯 5 및 7에 FC-VI 카드 전반에 8개의 FC-VI 포트가 필요합니다. 업그레이드를 시작하기 전에 AFF A700의 슬롯 7에 카드가 있는 경우 클러스터의 모든 노드에 대해 다른 슬롯으로 카드를 이동해야 합니다.



=== MetroCluster 구성의 상태를 확인합니다

AFF A900 패브릭 MetroCluster 구성의 RCF 파일 및 케이블을 업데이트하기 전에 구성 상태와 연결을 확인해야 합니다.
...
AFF A900 패브릭 MetroCluster에는 AFF A700에 필요한 4포트 FC-VI 어댑터 1개와 비교하여 노드당 4포트 FC-VI 어댑터 2개가 필요합니다. AFF A900 컨트롤러로 컨트롤러 업그레이드를 시작하기 전에 AFF A900 연결 토폴로지를 지원하도록 패브릭 스위치 RCF 파일을 수정해야 합니다.

. 에서 https://mysupport.netapp.com/site/products/all/details/metrocluster-rcf/downloads-tab["MetroCluster RCF 파일 다운로드 페이지"^]에서 AFF A900 패브릭 MetroCluster에 대한 올바른 RCF 파일과 AFF A700 구성에서 사용 중인 스위치 모델을 다운로드하십시오.
...
NOTE: AFF A900 패브릭 MetroCluster 구성을 지원하는 RCF 파일 업데이트는 AFF A700 패브릭 MetroCluster 구성에 사용되는 포트 및 연결에 영향을 미치지 않습니다.
...
=== AFF A700 노드의 포트를 AFF A900 노드로 매핑합니다

컨트롤러 업그레이드 프로세스 중에는 이 절차에서 설명하는 연결만 변경해야 합니다.

AFF A700 컨트롤러의 슬롯 7에 카드가 있는 경우 컨트롤러 업그레이드 절차를 시작하기 전에 카드를 다른 슬롯으로 이동해야 합니다. AFF A900에서 패브릭 MetroCluster를 작동하는 데 필요한 두 번째 FC-VI 어댑터를 추가하기 위해 슬롯 7을 사용할 수 있어야 합니다.
...
+
link:/tiebreaker/concept_configuring_the_tiebreaker_software.html["MetroCluster 구성을 제거합니다"]
...
이 작업을 완료한 후 site_a가 활성화되어 두 사이트의 데이터를 제공합니다. Site_B는 비활성 상태이므로 다음 그림과 같이 업그레이드 프로세스를 시작할 준비가 되어 있습니다.
...
== site_B에서 AFF A700 컨트롤러 모듈 및 NVS를 제거합니다
...
NOTE: AFF A700 NVS 모듈은 슬롯 6에 있으며 시스템의 다른 모듈에 비해 높이가 2배입니다.
...
NOTE: AFF A700 비휘발성 스토리지 모듈에 코어 덤프 장치로 사용되는 추가 모듈이 있는 경우 AFF A900 NVS로 전송하지 마십시오. AFF A700 컨트롤러 모듈 및 NVS의 부품을 AFF A900으로 전송하지 마십시오.



== AFF A900 NVS 및 컨트롤러 모듈을 설치합니다

Site_B의 두 노드에 있는 업그레이드 키트에서 AFF A900 NVS 및 컨트롤러 모듈을 설치해야 합니다 코어 덤프 장치를 AFF A700 NVS 모듈에서 AFF A900 NVS 모듈로 이동하지 마십시오.
...
다음 절차에 따라 site_B에서 두 노드의 슬롯 6에 AFF A900 NVS를 설치하십시오
...
+
.. 슬롯 7에서 스위치로 FC-VI 포트를 연결합니다. 을 참조하십시오 link:../install-fc/index.html["패브릭 연결 설치 및 구성"] 사용 중인 환경의 스위치 유형에 대한 AFF A900 패브릭 MetroCluster 연결 요구 사항을 참조하십시오.
...
AFF A900 컨트롤러 모듈과 NVS를 바꾼 후 AFF A900 노드를 netboot 하고 클러스터에서 실행 중인 것과 동일한 ONTAP 버전 및 패치 레벨을 설치해야 합니다. 네트워크 부팅이란 원격 서버에 저장된 ONTAP 이미지에서 부팅됨을 의미합니다. netboot를 준비할 때 시스템이 액세스할 수 있는 웹 서버에 ONTAP 9 부트 이미지 사본을 추가해야 합니다.
...
. AFF A900 컨트롤러 모듈에서 실행 중인 노드 1이 부팅될 때까지 기다린 후 다음과 같이 부팅 메뉴 옵션을 표시합니다.
+
[listing]
----
