...
image::../media/vip_and_bgp_in_metrocluster_2.png[MetroCluster 의 가상 IP 및 경계 게이트웨이 프로토콜]
...
구성이 완료되면 v0a(v0b 등)가 지속적으로 연결을 확인하여 LIF 마이그레이션 또는 페일오버가 성공하도록 보장합니다(파손된 구성이 운영 중단 후에만 표시됨).
