...
NOTE: 애그리게이트를 구성하기 전에 의 "MetroCluster 구성에서 FabricPool에 대한 오브젝트 저장소 설정"에 설명된 대로 오브젝트 저장소를 설정해야 합니다https://["디스크 및 애그리게이트 관리"]
...
ONTAP 9.11.1부터 https://["정합성 보장 그룹"^] MetroCluster 구성에서 지원됩니다.
