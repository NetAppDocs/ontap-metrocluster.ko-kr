...
LUN과 볼륨은 여전히 ​​기존 노드에 물리적으로 호스팅됩니다. LUN은 기존 노드 인터페이스에서만 보고되므로 호스트에는 기존 노드를 통한 경로만 표시됩니다. 이를 확인하려면 호스트에서 `sanlun lun show -p` 및 `multipath -ll -d` 명령을 실행하고 명령 출력을 검토하십시오.
