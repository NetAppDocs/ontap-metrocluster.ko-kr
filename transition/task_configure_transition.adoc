...
NOTE: 두 클러스터 중 하나에서 MetroCluster IP 인터페이스를 구성할 수 있습니다. 또한 ONTAP 9.9.1부터 계층 3 구성을 사용하는 경우 도 지정해야 합니다 `-gateway` MetroCluster IP 인터페이스를 생성할 때의 매개 변수입니다. 을 참조하십시오 link:../install-ip/concept_considerations_layer_3.html["계층 3 광역 네트워크에 대한 고려 사항"]
